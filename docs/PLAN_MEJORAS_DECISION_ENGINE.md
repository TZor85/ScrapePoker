...
### S2.3 — Facing bet penalty escalado por street (pendiente)
...
- **Fix:** Multiplicar penalty por factor de street: Flop ×1.0, Turn ×1.15, River ×1.3.
...
- **Fix:** Extender bluff catch a turn con condiciones más estrictas: `street == Turn && villainBetSize == Small && heroHandRank >= OnePair && equity >= FoldBelow * 0.90`.
...
- **Fix:** Requerir `(hasFlushDraw || hasComboDraw || totalOuts >= 6)` además de `totalOuts >= 4`.
...
- **Fix:** Antes de retornar bluff, verificar: `foldEquity >= betSizeAsRatio / (1 + betSizeAsRatio)`. Si fold equity es insuficiente, check en vez de bluff.
...
- **Ubicación:** `PostflopDecisionService.cs:422-434`
- **Fix:** Agregar case `"Monotone"` en el switch de baseBetThreshold con sizing `"Bet 1/4"`.
...
- **Fix:** Pasar `handSituation` al calculador preflop. Si hay VillainRange definido para la situación, usar Monte Carlo con ese rango en vez de random.
...
- **Fix:** Agregar `VillainBetSizeFlop: BetSizeCategory`, `VillainBetSizeTurn: BetSizeCategory`. Usar para ajustar facing bet penalties (ej: villain que escala sizing en turn = más fuerza).
...
- **Fix:** Analizar flop board como "initial danger state" (no como "change" sino como "base"). Propagar al turn/river para que `CombineBoardChanges` tenga contexto completo.
...
- **Ubicación:** `FrmMain.cs:1263, 1576, 1654` (parameter `villainStack: 0`)
- **Fix:** Extraer del jugador activo con mayor bet: `_playerGameState.Players.Where(p => p.Active && !p.IsHero).Max(p => p.Stack)`.
...
| Sprint 3 | S3.1-S3.5 | +5-8% ROI | Alto |
...
- `PostflopDecisionServiceTests.cs` — Tests para bluff condition fix (S1.1), combo draw bonus (S2.1), bluff catch turn (S3.1), facing bet scaling (S2.3)
- `ImpliedOddsCalculatorTests.cs` — Actualizar por cambios en calibración
- `PreflopAnalyzerTests.cs` — Tests para preflop vs VillainRange (S3.5)
